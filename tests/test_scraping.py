...
def _mock_response(markdown: str | None) -> SimpleNamespace:
    """Maak een nep-V1ScrapeResponse aan met een markdown-attribuut."""
...
def test_scrape_url_returns_markdown():
    ...
    with patch("src.scraping.V1FirecrawlApp") as mock_app_cls:
        mock_app_cls.return_value.scrape_url.return_value = _mock_response(
            "## inhoud\n\ntekst van de pagina"
        )
...
def test_scrape_url_uses_formats_kwarg():
    """scrape_url roept firecrawl aan met formats=['markdown'] als keyword-arg."""
    from src.scraping import scrape_url

    with patch("src.scraping.V1FirecrawlApp") as mock_app_cls:
        mock_app = mock_app_cls.return_value
        mock_app.scrape_url.return_value = _mock_response("x")
...
def test_scrape_url_empty_markdown_raises():
    """scrape_url raises ScrapingFailure als de response een lege markdown heeft."""
    from src.scraping import ScrapingFailure, scrape_url
...
def test_scrape_url_none_markdown_raises():
    """scrape_url raises ScrapingFailure als de response geen markdown-attribuut heeft."""
    from src.scraping import ScrapingFailure, scrape_url

    with patch("src.scraping.V1FirecrawlApp") as mock_app_cls:
        mock_app_cls.return_value.scrape_url.return_value = _mock_response(None)
        with pytest.raises(ScrapingFailure, match="lege"):
